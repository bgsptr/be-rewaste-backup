...
import {
  ...
  CarDto,
  IAssignCarDto,
} from 'src/application/dto/cars/car.dto';
...
class CarController {
  ...
  async createCarController(
    @Body(new ZodValidationPipe(assignCarSchema)) data: IAssignCarDto,
    @FetchJWTPayload() payload: { id: string; roles: string[] },
  ) {
    this.logger.debug(data);
    if (!payload.roles.includes(roleNumber.TRANSPORTER))
  ...
  async getFleetOfCarController(@FetchJWTPayload() payload: { id: string }) {
    const data = await this.carService.getFleetInformationWithDriverId(
      payload.id,
    );
  ...
  @Get('/:id')
  async getCarDetailController(@Param('id') id: string) {
    const car = await this.carService.getCarInformation(id);

    return {
      success: true,
      message: `successfully fetch car data with id ${car.id}`,
      data: {
        car,
      },
    };
