...
class UserService {
    ...
    async addVerificator(data: CreateVerificatorDto): Promise<string> {
        ...
        const mappedUser = this.verificatorMapper.toEntity({ villageId: null, ...restDto }, hashedPassword);
        ...
        try {
            const userId = await this.prisma.$transaction(async (tx) => {
                ...
                if (villageId) {
                    ...
                    if (village.userVerificatorId)
                        throw new CustomConflict('village', village.id, 'verificator already assigned to this village');
                }
            ...
            if (err instanceof PrismaClientKnownRequestError) {
                ...
                if (err.code === 'P2003') throw new CustomConflict('constraint error', '', 'foreign key violation');
            }
