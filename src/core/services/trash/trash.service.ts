...
class TrashService {
  ...
  async generateRegularPickups() {
    ...
    try {
      const activeCitizens =
        await this.userRepository.getAllCitizenHavingAddressAndNotRescheduled();
      ...
      const trashPickups = activeCitizens
        .filter((pickup) => pickup.villageId && pickup.address?.addressId)
        .map((pickup) => {
          const generatedTrashId = `WS-${generateIdWithNano()}`;
          return {
            verifyStatus: false,
  ...
  async getTrashById(trashId: string, userId: string) {
    try {
      ...
      const { userDriver, verification, trashTypes, ...rest } =
      ...
      // coba debug
      const verificator =
        verification?.verificatorUserId &&
        (await this.userRepository.getVerificatorDataById(
          verification.verificatorUserId,
        ));
      ...
      const data = {
        ...
        verificator: verificator
          ...
          : {},
        weightTotal: trashTypes.reduce(
          (total, tType) => total + tType.weight,
          0,
        ),
  ...
  async updateTrashStatusToNeedVerify(trashId: string, driverId: string) {
    try {
      const { id, pickupStatus, userDriver, userCitizen } =
        await this.trashRepository.getWithTypesById(trashId);
      if (pickupStatus !== PickupStatus.assigned)
        throw new CustomBadRequest(
          `failed to pickup, due the trash is not scheduled to any driver`,
        );
      if (userDriver && userDriver.userId !== driverId)
        throw new CustomForbidden(`cannot access anyone pickups`);

      await this.trashRepository.updatePickupStatusById(
        trashId,
        PickupStatus.verifying,
      );

      if (!userCitizen.villageId)
        throw new NotFoundException('trash associated with village');
      const verificatorUserId =
        await this.villageRepository.getAssignedVerificatorByVilageId(
      ...
      if (err instanceof PrismaClientKnownRequestError && err.code === 'P2025')
        throw new NotFoundException('trash', trashId);
      throw err;
    }
  }
  ...
  async updateStatusToSchedule(trashId: string, driverId: string) {
    try {
      // fix, verifikasi jika ada sampah terakhir yg diangkut gunakan titik itu, jika pertama kali ambil titik alamat driver
      const lastTrashPickup =
        await this.trashRepository.getAssignedLastForTodayWithDriverId(
          driverId,
        );
      const driver = await this.userRepository.getDriverById(driverId);
      const { id, pickupStatus, userCitizen, userDriver } =
        await this.trashRepository.getWithTypesById(trashId);
      if (userDriver)
        throw new CustomConflict('trash', '', 'trash already assigned');
      ...
      if (!userCitizen.address || !driver.address)
        throw new CustomBadRequest(
          'either citizen or driver did not add the address before',
        );
      ...
      const startLocation = lastTrashPickup
        ? {
            lat:
              (lastTrashPickup.userCitizen.address &&
                lastTrashPickup.userCitizen.address.lat) ||
              '',
            lng:
              (lastTrashPickup.userCitizen.address &&
                lastTrashPickup.userCitizen.address.lng) ||
              '',
          }
        : {
            lat: driverLat,
      ...
      if (!startLocation.lat || !startLocation.lng)
        throw new CustomBadRequest('');
      const result = await this.ors.calculateTwoPinpointWithDistanceAPI(
        startLocation,
      ...
      const estimatePickupAt = DayConvertion.getCurrent()
        .add(duration, 'second')
        .toDate();
      await this.trashRepository.updateTrashDetailById(
        driverId,
        trashId,
        distance,
        duration,
  ...
  async getDailyPickupList(userId: string) {
    ...
    if (!user?.driverVillageId || !user.transporterId)
      throw new NotFoundException('transporter');
    ...
    const trashList =
      await this.trashRepository.getTrashTodayFromSelectedVillage(
        linked.transporterVillage[0].villageId,
      );
    ...
    const listThatNeedVerify = trashList
      .filter((list) => list.userCitizen.loyaltyId && !list.verifyStatus)
      .map((item) => ({
        pickupIsFinished: 'need_verify',
        ...item,
      }));
    const needVerityTrashIds = new Set(
      listThatNeedVerify.map((item) => item.id),
    );
    const unloyaltyTrash = trashList
      .filter((item) => !needVerityTrashIds.has(item.id))
      .map((item) => ({
        pickupIsFinished:
  ...
  async getPickupListForDriverTimeline(driverId: string, dateString: string) {
    try {
